---
title: "Rapa Nui Mata'a Morphometric Analyses"
...
In exploring the the way in which *mata'a* forms vary, researchers have noted that there is a great diversity of shapes that vary from rounded to subangluar to angular to complex [@Mulloy:1961aa]. Early researchers assigned mata`a shape variation to what they conceived as ethnographic categories based on Rapanui words [i.e., @Routledge:1919aa].  Later attempts to construct systematic classifications have also focused on identifying types based on characterizations of overall shape.  None of these classification efforts produced useful categories.
...
Table 1:  *Mata'a* included in analyses by collection location.
...
Figure 3.  *Mata'a* included in the current analyses. The 5 colors indicate the collection locations on Rapa Nui (Blue=Ahu Tautiri, Green=Orito, Yellow=Orongo, Orange=Rano Kao, Red=Location only known to the level of the island). 
...
<!-- We don't need this now... using the static image. But perhaps at some point..
```{r polar, message=FALSE, echo=FALSE, warning=FALSE, fig.width=4,fig.height=4}
par(mfrow=c(1,1))
ci.plot(RapaNui)
...
![Figure 7.  Shape variability](../Figures/figure_1.png)

Figure 7.  Variability in *mata’a* shape shown with mean and 95% confidence intervals. Note that the 95% confidence intervals are shown with exaggerated differences between the values to illustrate areas with greater variance versus those with more constrained shape. Here, the area at the base of the stem where it meets the blade is the most constrained portion of *mata'a* shape while the stem length and blade are more variable. 
...
#b5 <- RapaNui %>% slice(1:12) ## sample 12 of them
#b5  %>% calibrate_harmonicpower_efourier(nb.h=12) #nb.h indicates # of harmonics

#updated for Momocs 1.3.2
library(ggplot2)
cal <- RapaNui  %>% calibrate_harmonicpower_efourier(nb.h=12)
cal$gg + theme_minimal() +
coord_cartesian(xlim=c(3.5, 12.5), ylim=c(0, 100)) +
ggtitle("Mata'a* reconstructions with harmonics")
```
...
```{r, rapaf-all-plot,echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
...
#plot(RapaF.p, pos.shp="full")
plot(RapaF.p, ~Site)
```

Figure 12.  First two principal components of *mata'a* grouped by site location. 

```{r, rapaf-bymaterial-plot, results='asis', echo=FALSE, cache=TRUE, warning=FALSE}
#, message=FALSE, fig.width=4.5,fig.height=4.5}
## By Source
...
The *mata'a* of Rapa Nui share some similarity with stone tools found on other islands across the Pacific.  On New Britain in Melanesia, for example, Torrence [-@Torrence:2009aa, -@Torrence:2009ab; -@Torrence:2013aa] has described obsidian stemmed tools that share much in common with the shape of *mata'a.* Torrence [see also @Kononenko:2012aa] argues that these tools may have been used for range of activities including tatooing and ritual scarification.  Pitcairn Island is remote Eastern Polynesian island that is located ca. 1900 km to the west of Rapa Nui. The island was inhabited by historically related Polynesian populations, though abandoned during prehistory.  Stemmed lithics of chert known as locally as "mataa" are also found on the Chatham Islands and on New Zealand [@Jones:1981aa]. An additional but limited comparison can be made with Pitcairn Island, where a few stemmed lithic tools have been found [@Heyerdahl:1965aa]. 

As a comparison for our study, we generated outlines of examples stemmed lithic tools from published images and drawings of these islands using the same procedure for Rapa Nui. Our elliptic Fourier analyses of shape variability, required 13 harmonics to adequately characterize the shape of all of the stemmed artifacts.  While the sample sizes of the non-Rapa Nui assemblages are small, when we compare the shapes of Rapa Nui *mata'a* with those other objects, we find that the Pitcairn Island stemmed artifacts have overall shapes that are quite distinct. While we cannot rule out the possibility that the Pitcairn examples are a few extremely long and pointed shapes that happen to have been collected from a much wider array of variability, these shapes are certainly more consistent with hafted tools for hunting or weapons.  The New Zealand mataa are similar but have substantially thicker stems than the Rapa Nui artifacts.  Jones [-@Jones:1981aa] suggests that this might reflect tools that are hafted with the edge perpindicular to the shaft like an adze.  Given their chert composition and relatively steep edge angles, this shape might be well suited for wood working. 
...
```{r, allfp-plot,echo=FALSE, cache=TRUE, warning=FALSE, message=FALSE}
#,  fig.width=4.5, fig.height=4.5 } 
#plot(PCA(allF), "Island", ellipses=TRUE)
plot(PCA(allF), ~Island)
...
```{r bootstrap, message=FALSE, echo=FALSE, warning=FALSE, fig.width=4.5,fig.height=4.5}
par(mfrow=c(1, 2))
