...
distance <- array(dim=c(nrow(xy), ncol(xy)))
for (i in 1:nrow(xy)) {
  for (j in 1:ncol(xy)) {
    distance[i,j] = sqrt( (xy[i,seq(1,ncol(xy),1)] - xCenter)^2 + (xy[i,seq(1,ncol(xy),2)]-yCenter)^2)/75
    }
  }
...
for (i in 1:ncol(xy)) {
  ...
  upperCI[i]<-confidenceInterval[i].upper
  lowerCI[i]<-confidenceInterval[i].lower
}
