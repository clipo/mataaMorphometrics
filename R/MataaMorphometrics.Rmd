...
PAST <- PAST[which(allPAST$Island=="Rapa Nui"),]
...
for (i in 1:nrow(scaledXY)){ 
  ...
  allcoo[[i]] <- cbind(scaledXY[i, seq(1, ncol(scaledXY), 2)], scaledXY[i, seq(2, ncol(scaledXY), 2)])
  }
...
```
Figure 4.  *Mata'a* included in the current analyses.
...
While the range of variability in shape shown in Figure 4 is substantial, simple metrics of length and width (Figure 5) suggest that there is just a single distribution of these objects without clear-cut modes in size. Comparisons of length and width, however, are fairly crude descriptions of shape.  

```{r stack, message=FALSE, echo=FALSE, warning=FALSE, fig.width=4.5,fig.height=4.5}
# Lets stack them scaling them 
stack(RapaNui)
```
We can examine variabilty 
...
par(mfrow=c(1, 2))
```
Figure 6.  Variability in *mata’a* shape shown with 95% confidence intervals and quartiles. Note that the 95% confidence intervals are shown with exaggerated differences between the values to areas with greater variance versus those with more constrained shape.
