...
#first calculate the angle and distance that each point sits on from the centroid for each mataa
curangle <- 0

angles<-(dim=c(nrow(scaledXY), ncol(scaledXY)/2))
rads<- array(dim=c(nrow(scaledXY), ncol(scaledXY)/2))
degrees<- array(dim=c(nrow(scaledXY), ncol(scaledXY)/2))
distance <- array(dim=c(nrow(scaledXY), ncol(scaledXY)/2))
for (i in 1:nrow(xy)) {   
  rads[i,] <- atan2(   -(yCenter - scaledXY[i,seq(1,ncol(scaledXY),2)]), (xCenter - scaledXY[i,seq(2,ncol(scaledXY),2)])  )
  degrees[i,] <- rads[i] * (180 / pi);
  distance[i,] <- sqrt(  (scaledXY[i,seq(1,ncol(scaledXY),2)]-xCenter)^2 
                      + (scaledXY[i,seq(2,ncol(scaledXY),2)]-yCenter)^2 )
  a <- 0
  for (d in degrees[i,]) {
        a = a + 1
        angles[i,d ] <- distance[i,a]
  }
}

anglesAndDistances <- array(dim=c(nrow(scaledXY), ncol(scaledXY)/2))
# now for all 360 degrees find the closest point for each object at that angle and pool together the distances
for (a in seq(1,360)){
  for (i in 1:nrow(xy)){
    anglesAndDistances[a,]<-  which(abs(list[i,]-a)==min(abs(abs(list[i,]-a))))
  }
}
