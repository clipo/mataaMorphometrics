...
xCenter = 900/scalingFactor  # x coordinate of centroid (if its fixed, otherwise can be aligned based on average centroid, etc.)
yCenter = 800/scalingFactor # y coordinate of centroid
...
xMaxWidth<-0
for (i in 1:nrow(xy)){ 
  xMaxLength[i]<-(max(xy[i, seq(2, ncol(xy), 2)])-min(xy[i, seq(2, ncol(xy), 2)]) )/scalingFactor
  xMaxWidth[i]<-(max(xy[i, seq(1, ncol(xy), 2)])-min(xy[i, seq(1, ncol(xy), 2)]) )/scalingFactor
  }
...
curangle <- 0
angles<-0
for (i in 1:(ncol(xy))/2) { 
  curangle <- curangle + 360/(ncol(xy))
  angles[i]<-curangle 
}
...
for (i in 1:nrow(xy)) {
    distance[i,] = sqrt((xy[i,seq(1,ncol(xy),2)] - xCenter)^2 + (xy[i,seq(2,ncol(xy),2)]-yCenter)^2 )/scalingFactor
}

## now calculate the mean, sd, upper and lower CI for the distances from centroid (we will use this in a polar graph)
averageDistances <- 0
sdDistance <- 0
...
for (i in 1:ncol(distance)) {
  averageDistances[i]<-mean(distance[,i])
  sdDistance[i]<-mean(distance[,i])
  confidenceInterval<-CI(distance[,i])
  upperCI[i]<-confidenceInterval[1]
  lowerCI[i]<-confidenceInterval[3]
}
...
polar.plot(averageDistances,angles,pos=NULL,label.pos=NULL, start=0,clockwise=TRUE,rp.type="l",line.col=4) 
polar.plot(upperCI,angles,pos=NULL,label.pos=NULL, start=0,clockwise=TRUE,rp.type="l",line.col=3) 
polar.plot(lowerCI,angles,pos=NULL,label.pos=NULL, start=0,clockwise=TRUE,rp.type="l",line.col=5) 
```
