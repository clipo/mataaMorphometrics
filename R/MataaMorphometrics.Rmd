...
hpow(RapaNui, method = "efourier", title="Fourier coefficients power spectrum", plot.method="stack")
```
...
The *mata'a* of Rapa Nui share some similarity with stone tools found on other islands across the Pacific.  On New Britain in Melanesia, for example, Torrence (-@Torrence@2009aa, -@Torrence@2009ab, -@Torrence@2013aa) has described obsidian stemmed tools that share much in common with the shape of *mata'a.* Torrence (see also @Kononenko:2012aa) that these tools may have been used for range of activities including tatooing.  Pitcairn Island is remote Eastern Polynesian island that   is located ca. 1900 km to the west of Rapa Nui. The island was inhabited by historically related Polynesian populations, though abandoned during prehistory.  A few stemmed lithic tools have been found on Pitcairn (@Heyerdahl:1965aa). 

As a comparison for our study, we generated outlines of examples stemmed lithic tools from published images and drawings of these islands using the same procedure for Rapa Nui. While the sample sizes of the non-Rapa Nui artifacts are small, when we compare the shapes of *mata'a* with those other objects, we find that the Pitcairn Island stemmed artifacts have overall shapes that are quite distinct.  The New Britain artifacts that are thought to represent an array of non-lethal function have shape features that are more like the ones from Rapa Nui.
